...
@implementation PostStentMIP

- (id) initWithPixList:(NSMutableArray*)p;
{
    
    DCMPix *pix1 = [p objectAtIndex:0];
    //DCMPix *pix1 = [[DCMPix alloc] initWithContentsOfFile:[[p objectAtIndex:0] sourcefile]];
    
    height = [pix1 pheight];
    width = [pix1 pwidth];
    pixelSpacingX = [pix1 pixelSpacingX];
    pixelSpacingY = [pix1 pixelSpacingY];
    originX = [pix1 originX];
    originY = [pix1 originY];
    originZ = [pix1 originZ];
    
    float mip[height*width];
    ...
    for (DCMPix *pix in p)
    {
        ...
        for (int y = 0; y < height; y++) 
        {
            ...
            {
                float pixel = [pix getPixelValueX:x Y:y];
                
                if (pixel < mip[counter]) 
                {
                    mip[counter] = pixel;
    ...
    NSLog(@"MENOR1 = %f", menor); 
    
    self = [self initWithData:mip :32 :height :width :pixelSpacingX :pixelSpacingY :originX :originY :originZ];
    
    [self imageArithmeticSubtraction:pix1 absolute:FALSE]; 
    ...
    for (int y = 0; y < height; y++) 
    {
        for (int x = 0; x < width; x++) 
        {
            float pixel = [self getPixelValueX:x Y:y];
    ...
    NSLog(@"MENOR2 = %f", menor2);
    
    //If there are negative values, we insert an offset in order to being able to apply the filters.
    if (menor2 < 0)
    {
        float *data = [self fImage];
    ...
    [self setfImage:[self subtractImages:[self fImage] :temp]];
    ...
    //[self setfImage:temp];
    //Para comprobar que realmente estamos actuando sobre el vector mip
    ...
    if (menor2 < 0)
    {
        pixels = [[NSMutableArray alloc] initWithArray:[self pixArray]];
        //NSMutableArray *pixelsModified = [[NSMutableArray alloc]initWithCapacity:[pixels count]];
        //NSNumber *pixel = [[NSNumber alloc] initWithFloat:0];
        //NSNumber *pixel;
        
        for (int i = 0; i < [pixels count]; i++) 
        {
            float pixel = [[pixels objectAtIndex:i] floatValue] - menor2;
            NSNumber *pixelNumber = [[NSNumber alloc] initWithFloat:pixel];
            [pixels replaceObjectAtIndex:i withObject:pixelNumber];
    ...
    //float min = [self minValueOfSeries]
    [self changeWLWW:fullwl :fullww];
    
    float windowwidth = [self ww];
    float windowlevel = [self wl];
    
    NSLog(@"WINDOW WIDTH: %F", windowwidth);
    NSLog(@"WINDOW LEVEL: %f", windowlevel);
...
{
    NSLog(@"We are in applyFiltersWithAlpha.");
    ...
    origin[0] = [self originX];
    origin[1] = [self originY];
    origin[2] = [self originZ];
    
    [self orientationDouble: vectorOriginal];
    ...
    PostStentFilteredImage *imagenFiltrada = [[PostStentFilteredImage alloc] initWithData:resultBuff :32 :size[0] :size[1] :spacing[0] :spacing[1] :origin[0] :origin[1] :0];
